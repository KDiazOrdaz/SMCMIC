...
# Model: specify the file with the model giving the correct path, 
#here assumed it is at the top of the project directory
mfile <- "../../../../BayesMixModel_binoutcome_aux.txt"




while (!file.exists(paste("results/BayesUnobsX.missy.",taskID,"to",(taskID+everyout-1),".RData",sep="")))
  {
   ...
   dta.df<-dta.df[-c(1,3,6,8)]
    names(dta.df)<-c("y", "x2", "rnd","cmp" )
    set.seed(1001)
    ...
    rnd <- dta.df$rnd
    cmp <- dta.df$cmp
    #prin_cmp <- dta.df$prin_cmp
    #x1 <- dta.df$x1
    x2 <- dta.df$x2
    
    data.list <- list("N.obs", "y",  "rnd", "cmp", "x2")
...
r.med.cace[[count]]<-med.cace
  ...
  save(res,file=paste("results/BayesUnobsX.missy.",taskID,"to",(taskID+everyout-1),".RData",sep=""))
