DGP.bin<-function(
  ...
  beta.c= 0, # effect of complying in the control arm, change to beta.tc/2 for the other simultion
...
##outcome beta0=0, no effect of complying in the control group (if first coef 0), no effec of randomisation
eta.y<-{beta.c}*prin.cmp + beta.x1*x1+beta.x2*x2+ beta.tc*prin.cmp*rnd   
pi.y<-exp(eta.y)/(exp(eta.y)+1)
y<-rbinom(N, 1,pi.y)
